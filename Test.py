...
class DeltaPopLocaties(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.entry_search = TextInput()
        self.add_widget(self.entry_search)

        self.button_search = Button(text="Zoeken")
        self.button_search.bind(on_press=self.search_excel)
        self.add_widget(self.button_search)

    def search_excel(self, instance):
        search_query = self.entry_search.text
        if not search_query:
            popup = Popup(title='Error', content=Label(text='A.U.B Type de naam van de pop.'), size_hint=(None, None),
                          size=(400, 200))
            popup.open()
            return
        ...
        try:
            ...
            if result:
                ...
                popup = Popup(title='Search Result', content=ScrollView(size_hint=(1, None), size=(400, 400)),
                ...
                if 'Adres' in result_dict:
                    ...
                    button_layout = BoxLayout(orientation='horizontal', size_hint=(None, None), size=(400, 50))
